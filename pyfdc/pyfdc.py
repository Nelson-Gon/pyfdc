...
class FoodDataCentral(object):
    ...
    def __init__(self, api_key=None):
        ...
        if api_key is None:
            self.api_key = os.environ.get(
                "pyfdc_key") if "pyfdc_key" in os.environ else key_signup()
        else:
    ...
    def get_food_info_internal(self, search_phrase=None,
        ...
        try:
            url_response = requests.get(self.base_url, params=search_query, headers={
            ...
            for x in unprocessed_result:
                res.append([val for key_id, val in x.items() if key_id == self.available_targets[target]])
            return res 

    def get_food_info(self, search_phrase=None,
                      ingredients=None, brand_owner=None, page_number=1,
                      page_size=50,
                      sort_field=None, sort_direction='asc', target_fields=None):
        """
        ...
        if not isinstance(target_fields, (list, tuple)):
            raise TypeError(
                f"target should be a list or tuple not {type(target_fields).__name__}")

        try:
            result = Parallel(os.cpu_count() - 1, backend="loky")(delayed(
                self.get_food_info_internal)(search_phrase,
                ingredients,
                brand_owner,
                page_number,
                page_size,
                sort_field,
                sort_direction, target) for target in target_fields)
        except KeyError:
            raise KeyError(
                f"target_key should be one of {self.available_targets.keys()}")
    ...
    def get_food_details(self, fdc_id=None, target_field=None, result_format="full", nutrients=None):
        ...
        try:
            ...
            assert isinstance(
                fdc_id, int), f"fdc_id should be an int not {type(fdc_id).__name__}"
            ...
            if target_field is None:
                ...
                if target_field == "label_nutrients":
                    if not "labelNutrients" in result.keys():
                        raise KeyError(
                            f"FDC ID {fdc_id} has no label nutrients.")
                    label_nutrients_df = json_normalize(
                        result["labelNutrients"])
                    label_nutrients_df.columns = [
                        re.sub(".value", "", x) for x in label_nutrients_df]
